const CACHE_NAME='financas-cn-v57';
...
self.addEventListener('fetch',event=>{
  ...
  if(url.origin===self.location.origin)event.respondWith(caches.match(request).then(cached=>cached||fetch(request).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy));return response})));
});
